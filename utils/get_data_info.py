...
def enumerate_labels(path_data, table_scored):
            ...
            for line_idx, line in enumerate(file):
                if line_idx == 15:
                    ...
                    for word in list_of_labels:
                        word_pom.append(int(word) in codes_int)
                    # labels_presence.append(word_pom)
                    labels_pom = os.path.split(file_n)
    ...
    with open('tables/true_false_labels_scored_alldata.json', 'w') as fp:
...
def sub_dataset_labels_sum(list_of_paths):
    ...
    with open('tables/true_false_labels_scored_alldata.json') as fp:
