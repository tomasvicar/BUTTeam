...
def evaluate_12ECG_score(label_directory, output_directory):
    ...
    nans=np.isnan(np.mean(output_probabilities,axis=1))
    
    labels=labels[nans==0,:]
    output=output[nans==0,:]
    output_probabilities=output_probabilities[nans==0,:]
    print('nans:' + str(np.sum(nans)/len(nans)))
